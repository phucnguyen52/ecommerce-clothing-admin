...
import { Button, Col, Form, Input, InputNumber, Row, Select, Upload, Modal, } from "antd";
...
const ModalCategory = (props) => {
   ...
   const onFinish = async (values) => {
      ...
      try {
         const req = await fetch(`http://localhost:8080/api/admin/category${type === "update" ? `/${data.id}` : ''}`, {
               ...
               >
                  ...
                  {imageUrl && (
                     <div className="w-[150px] h-[200px] relative">
                        ...
                        />
                        <div className="absolute top-0 right-1.5 cursor-pointer" onClick={() => DeleteImg()}>
                           <CloseOutlined />
                        </div>
                     </div>
                  )}

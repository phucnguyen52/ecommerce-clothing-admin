...
const Voucher = () => {
    ...
    const fetchVoucher = async () => {
        try {
            const req = await fetch(`http://localhost:8080/api/admin/voucher${status ? `?status=${status}` : ''}`);
                                ...
                                data.map((data, i) => (
                                    ...
                                    >
                                        <td className="px-6 py-2 bg-gray-50 dark:bg-gray-800">
                                            <div className="text-blue-900 text-sm">{data.nameVoucher}</div>
                                        </td>
                                        <td className="px-6 py-2 font-medium text-gray-900 whitespace-nowrap bg-gray-50 dark:text-white dark:bg-gray-800">
                                            {data.discountCode}
                                        </td>
                                        <td className="px-6 py-2 ">
                                            <div className={`${data.status === 'active' ? 'bg-green-400' : (data.status === 'stop' ? 'bg-gray-400' : 'bg-blue-300')} p-1 text-center rounded-2xl`}>
                                                {data.status === "active" ? "Hoạt động" : data.status === "stop" ? "Ngừng" : "Bản nháp"}
                                        ...
                                        <td className="px-6 py-2">
                                            <div className="text-xs italic text-center">{format(new Date(data.startDate), 'dd/MM/yyyy')} - {format(new Date(data.endDate), 'dd/MM/yyyy')}</div>
                                        </td>
                                    </tr>
                                ))
                            ) : (
                ...
                {selectedVoucher && (
                    <div className="w-1/3 shadow-md rounded-lg text-lg bg-white p-4">
                        ...
                        <div className="flex gap-1 my-4 flex-col">
                            ...
                            <div className="italic text-base">
                                {format(new Date(selectedVoucher.startDate), 'dd/MM/yyyy')} - {format(new Date(selectedVoucher.endDate), 'dd/MM/yyyy')}
                            </div>
                        </div>
                        <div className="flex gap-1 my-4 flex-col">

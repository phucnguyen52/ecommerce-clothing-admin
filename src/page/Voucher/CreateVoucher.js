...
const CreateVoucher = () => {
    ...
    const fetchUser = async (search) => {
        try {
            const req = await fetch(`http://localhost:8080/api/admin/user/search?search=${search}`)
            const res = await req.json()
            if (res.success) {
                setUsers(res.user)
            }
    ...
    const handleSelectUser = (u) => {
        setUsers([])
        setSearchValue('');
        const filteredUsers = user.length > 0 ? user.filter(user => user.id === u.id) : [];
        if (filteredUsers.length === 0) setUser([...user, { id: u.id, fullName: u.fullName }]);
    }
    ...
    const handleCreate = async(e) => {
        ...
        const category = isCategory ? [...values.category].filter(i => i.checked === true).map(i => i.value) : []
        ...
        const voucher = {
            ...
            quantityDiscount: values.count.value,
            ...
            <form onSubmit={handleCreate}>
                <div className="flex justify-between items-center py-4 pl-3 bg-gray-100/80 border-b-[1px] relative">
                    ...
                    <div className="absolute top-4 right-4 flex gap-5">
                        <button type="submit" name="draft" onSubmit={handleCreate} className="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">
                            LƯU BẢN NHÁP
                        </button>
                        <button type="submit" name="active" onSubmit={handleCreate} className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2">
                            LƯU VÀ KÍCH HOẠT
                        </button>
                    </div>
                </div>
                            ...
                            <div className="py-4">
                                ...
                                {user?.length > 0 && (
                                    <table className="min-w-full table-auto   bg-white">
                                        <thead>
                                            <tr >
                                                <th className="px-4 py-2 text-left font-semibold border-b">STT</th>
                                                <th className="px-4 py-2 text-left font-semibold border-b">Họ tên</th>
                                                <th className="px-4 py-2 text-center border-b hover:cursor-pointer" onClick={() => setUser([])}>Xoá tất cả</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {user.map((u, index) => (

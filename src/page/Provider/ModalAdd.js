    ...
    useEffect(() => {
        const loadDataRow = async () => {
            ...
            if (dataRow && provinces.length > 0) {
                ...
                const addressComponents = dataRow.address.split(", ");

                let xa = addressComponents[1];
                let huyen = addressComponents[2];
                let tinh = addressComponents[3];
                const provinceIndex = provinces.findIndex(
                    (item) => item.province_name === tinh
                );

                if (provinceIndex !== -1) {
                    const selectedProvinceId =
                        provinces[provinceIndex].province_id;
                    setProvince(selectedProvinceId);

                    const districtResponse = await apiGetPublicDistrict(
                        selectedProvinceId
                    );
                    const districtData = districtResponse.data?.results;
                    setDistricts(districtData);

                    const districtIndex = districtData.findIndex(
                        (item) => item.district_name === huyen
                    );
                    if (districtIndex !== -1) {
                        const selectedDistrictId =
                            districtData[districtIndex].district_id;
                        setDistrict(selectedDistrictId);

                        const wardResponse = await apiGetPublicWard(
                            selectedDistrictId
                        );
                        const wardData = wardResponse.data?.results;
                        setWards(wardData);

                        const wardIndex = wardData.findIndex(
                            (item) => item.ward_name === xa
                        );
                        if (wardIndex !== -1) {
                            setWard(wardData[wardIndex].ward_id);
                        }
                    }
                }
                setLoading(false);
    ...
    const handleUpdateProvider = async (supplierInfo, dataRow) => {
        if (supplierInfo.address || supplierInfo.phone || supplierInfo.name) {
            ...
            if (supplierInfo.address)
                dataToUpdate.address = supplierInfo.address;
            if (supplierInfo.phone)
                dataToUpdate.numberPhone = supplierInfo.phone;
    ...
    return (
        <div>
            ...
            {loading && title === "Cập nhật nhà cung cấp" ? (
                <div
                    role="status"
                    className="flex items-center justify-center p-40"
                >
                    <svg
                        aria-hidden="true"
